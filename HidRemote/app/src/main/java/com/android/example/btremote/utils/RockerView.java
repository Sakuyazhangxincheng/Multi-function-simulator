...
public class RockerView extends View {
    ...
    //移动点坐标
    private float moveX, moveY, upX, upY;
    //中心点点坐标
    private int cx, cy;
...
//            //Log.i(TAG, "RockerView: isInEditMode");
    ...
    interface OnRockListener {

       void onMove(double d);
    }
    private OnRockListener listener;
    void setOnRockListener(OnRockListener listener){
    ...
    private void initAttribute(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.RockerView2);

        // 可移动区域背景
        Drawable areaBackground = typedArray.getDrawable(R.styleable.RockerView2_areaBackground);
        ...
        // 摇杆背景
        Drawable rockerBackground = typedArray.getDrawable(R.styleable.RockerView2_rockerBackground);
...
//                upY = event.getY();
     ...
     * @param angle 摇动角度
     */
    public void callBack(double angle) {
        if (null != mOnAngleChangeListener) {
            mOnAngleChangeListener.angle(angle,moveX,moveY);
...
//                        if (moveX - moveY > 100 || moveY - moveX > 100 || upX - upY > 100 || upY - upX > 100)
    ...
    public interface OnAngleChangeListener {
         ...
         */
        void angle(double angle,double x,double y);
        // 结束
        void onFinish();
    }
}
